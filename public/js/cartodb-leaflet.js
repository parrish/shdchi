...
if (typeof(L.CartoDBLayer) === "undefined") {
  ...
  L.CartoDBLayer = function (params) {
    ...
    if (this.params.infowindow || this.params.interactivity) {
      ...
      params.tilejson = generateTileJson(params);
        ...
        //HERE IS WHERE YOU CAN INTERCEPT THE EVENTS    
        //LOOK HERE
          out: function(){
          	$('div.over').hide();
          ...
          click: function(feature, div, op3, evt) {
	          ...
	          openfeature(feature);
	          //DO NOT DISPLAY THE DEFAULT INFOWINDOW
    ...
    function generateTileJson(params) {
      ...
      if (params.query) {
        ...
        //Hack for hexagons
        //base_url = base_url.replace(/%7Bx%7D/g,"{x}").replace(/%7By%7D/g,"{y}").replace(/%7Bz%7D/g,"{z}");
      ...
      if (params.interactivity) {
      	grid_url = wax.util.addUrlData(grid_url, 'interactivity=' + params.interactivity);
      }
      ...
      return {
        ...
        formatter: function(options, data) {
            return data[params.interactivity] || data.cartodb_id;
        }
      };
    }
...
L.CartoDBInfowindow = L.Class.extend({
	...
	_update: function() {
		...
		//console.log(this.options.interactivity);

		if (this.options.interactivity==undefined) {
			var that = this
	    ...
	    $.ajax({
		    ...
		    timeout: 2000,
		    callbackParameter: 'callback',
		    success: function(result) {onSuccess(result.rows[0])},
	    	error: function(e, msg) {
	    		if (that.options.debug) throw('Error retrieving infowindow variables: ' + msg);
	      }
	    });
		} else {
			var label = (that.options.interactivity[0]).toString();
			//console.log(label);
			onSuccess(new Object({'magnitude':this._feature}));
		}
	},
